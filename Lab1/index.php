...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($api_key_correct){
        ...
        //use setters and getters to assign the values
        ...
        if($res){
            ...
            $response_array = ['success' => 0, 'message' => 'Wrond API key!'];
            header('Content=type: application/json');
            echo json_encode($response_array);
        }
    }else if($_SERVER['REQUEST_METHOD'] === 'GET'){
        //retriving the order status
    }else{
        //sorry we are not supporting this right now
    }
